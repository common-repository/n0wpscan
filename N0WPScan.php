...
Version: 5.6
Author: GeekParadize.Fr
Author URI: https://www.geekparadize.fr

N0WPScan - Prevent WPScans on your site, disable this plugin when you are security testing or looking for vulnerabilities!
...
add_option("nowpscan_total_scans", 0);

// Register a Scan to the Database
function nowpscan_registerScanToDb(){
	$scan_count = get_option("nowpscan_total_scans");
	$new_count = intval($scan_count)+1;
	update_option("nowpscan_total_scans", $new_count);
}

// Create Admin Notice
function nowpscan_admin_scan_notice() {
	$scan_count = intval(get_option("nowpscan_total_scans"));
	if ($scan_count === 0) {
		$nowpscan_status = "Good news, no direct WordPress application scans yet, we will keep watching.";
	} elseif ($scan_count === 1) {
		$nowpscan_status = "NOWPScan prevented " . $scan_count . " scan on your website.";
	} else {
		$nowpscan_status = "NOWPScan prevented " . $scan_count . " scans on your website.";
	}

    ?>
		<img src="<?php echo plugin_dir_url( __FILE__ ); ?>nowpscan.png" style="width: 200px;" /><br />
		<p><strong><?php echo $nowpscan_status; ?></strong></p><br />
    <?php
}

add_action( 'wp_dashboard_setup', 'nowpscan_nowpscan_widget' );
function nowpscan_nowpscan_widget() {
	wp_add_dashboard_widget(
		'nowpscan',
    	'NOWPSCAN',
		'nowpscan_admin_scan_notice'
  	);

	if (isset($wp_meta_boxes)) {
		// Then we make a backup of your widget.
		$wpblockwidget = $wp_meta_boxes['dashboard']['normal']['core']['nowpscan'];
		// We then unset that part of the array.
		unset($wp_meta_boxes['dashboard']['normal']['core']['nowpscan']);
		// Now we just add your widget back in.
		$wp_meta_boxes['dashboard']['side']['core']['nowpscan'] = $nowpscanwidget;
	}
}


// Add notice to admin notices
//add_action( 'admin_notices', 'nowpscan_admin_scan_notice' );

// Check to see if '?advanced_fingerprinting=' is set
if (isset($_GET['advanced_fingerprinting'])) {
	status_header(404);
	nowpscan_registerScanToDb();
	die();
}
...
add_action('init', 'nowpscan_nowpscan_init');
function nowpscan_nowpscan_init() {

	global $wp_version;
	$transient_name = 'nowpscan_'.$_SERVER['REMOTE_ADDR'];
...
add_action('do_robots', 'nowpscan_nowpscan_do_robots', 1);
function nowpscan_nowpscan_do_robots() {
	nowpscan_registerScanToDb();
	status_header(404);
	die();
}

//function nowpscan_add_fake_xmlrpc() {
...
add_filter('wp_xmlrpc_server_class', 'nowpscan_nowpscan_fake_xmlrpc');
class nowpscan_nowpscan_fake_xmlrpc {
...
add_filter('the_generator', 'nowpscan_nowpscan_remove_generator');
//function nowpscan_nowpscan_remove_generator() {
//    return '';
//}

register_activation_hook( __FILE__, 'nowpscan_nowpscan_activation');
function nowpscan_nowpscan_activation() {
	add_filter('rewrite_rules', 'nowpscan_rewrite_rules_filter');
	function nowpscan_rewrite_rules_filter($rules){
			$exploded = explode("\n", $rules);
			$my_rules = array(
				'# SECURITY HEADERS',
                ...
                'RewriteCond %{HTTP_USER_AGENT} ^Grafula [OR]',
								'RewriteCond %{HTTP_USER_AGENT} ^hackney [OR]',
...
register_deactivation_hook( __FILE__, 'nowpscan_nowpscan_deactivation');
function nowpscan_nowpscan_deactivation() {
	flush_rewrite_rules(true);
}
...
$nowpscan_required_capability = 'edit_others_posts';
...
$nowpscan_redirect_to = '';

// To make upgrades easier, you can set these in wp-config.php like:
/*
define('NOWPSCAN_REQUIRED_CAPABILITY', 'edit_others_posts');
define('NOWPSCAN_REDIRECT_TO' , 'http://enter-url-here');
*/

// Override these values from the constants if they are defined and not empty
if (defined('NOWPSCAN_REQUIRED_CAPABILITY'))
	$nowpscan_required_capability = NOWPSCAN_REQUIRED_CAPABILITY;
if (defined('NOWPSCAN_REDIRECT_TO'))
	$nowpscan_redirect_to = NOWPSCAN_REDIRECT_TO;

if (!function_exists('nowpscan_init')) {
	function nowpscan_init() {
		// We need the config vars inside the function
		global $nowpscan_required_capability, $nowpscan_redirect_to;
		...
		if ( stripos($_SERVER['REQUEST_URI'],'/wp-admin/') !== false && stripos($_SERVER['REQUEST_URI'],'async-upload.php') == false && stripos($_SERVER['REQUEST_URI'],'admin-ajax.php') == false ) {
			if (!current_user_can($nowpscan_required_capability)) {
			...
			//if (!is_site_admin()) {
				...
				if ($nowpscan_redirect_to == '') { $nowpscan_redirect_to = get_option('home'); }
...
add_action('init','nowpscan_init',0);
